...
Chrome или Edge 116+: расширенный режим картинки в картинке построен на
Document Picture-in-Picture API, а он есть только в них.
...
в `PATH`. macOS, Windows, Linux. `start.command`, `start.bat` и `start.sh`
запускают сервер и открывают страницу по двойному клику.
...
- Браузерный режим: окно без полосы с адресом, контролы системы. Предыдущая
  и следующая серии работают через Media Session API.
- Расширенный режим: в плавающее окно переезжает весь плеер с полосой
  перемотки, громкостью, скоростью, выбором звуковой дорожки и субтитров,
  повтором и автопереходом. Очередь, настройки и полный экран остаются
  в основном окне.
...
- Один проход ffmpeg на файл и дорожку: видео копируется потоком,
  перекодируется только выбранная звуковая дорожка, в AAC.
...
- Сортировка по имени, обратный порядок, перемешивание, ручное изменение
  порядка файлов.
- Два вида списка: строки с длительностью, размером и числом дорожек или сетка
  кадров из самих файлов.
...
- Общие настройки плеера под шестерёнкой справа вверху: панель файлов
  перекрывает видео или сужает его, ручное изменение порядка файлов, скрытие
  панели управления при автопереходе, шрифт, язык. Там же список горячих
  клавиш и кэш сервера.
- Кэш показан одной полосой: заливка показывает занятое место, ручка задаёт
  ...
  для кэша на диске: занятое им плюс свободное. Меньше 8 ГБ предел не
...
| `Esc` | закрыть меню, окно PiP или очередь |
...
test/           тесты
assets/         картинки для README и логотип
...
запускает 106 тестов: сервер по HTTP, интерфейс в Chrome без окна. Медиа для
тестов собирает ffmpeg во временной папке, ваши файлы не читаются. Нужны
Node 20+, ffmpeg и Chrome; без Chrome тесты интерфейса пропускаются.
